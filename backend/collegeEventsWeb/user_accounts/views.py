...
class RegisterView(APIView):
    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            # ONLY generate tokens for ACTIVE users (students)
            if user.status == "active":
                refresh = RefreshToken.for_user(user)
                access = str(refresh.access_token)
                ...
                # For pending organizers, return success without tokens
                return Response({
                    'user': UserSerializer(user).data,
                    'detail': 'Registration successful. Account pending admin approval.'
                }, status=status.HTTP_201_CREATED)
...
class RejectOrganizerView(APIView):
    ...
    def post(self, request, user_id):
        ...
        try:
            ...
            user.status = 'rejected'  # Changed from 'suspended'
            user.is_active = False
            user.save()
